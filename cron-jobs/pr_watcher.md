...
INITIAL TRIAGE LABELING RULE:
When triaging any open PR (dependabot or third-party), check its current `labels`. Ensure the PR has at least one classification label used for release notes (`bug`, `enhancement`, `documentation`, `dependencies`, `chore`, `cleanup`, `breaking`, `github_actions`). If it lacks a classification label, inspect the PR title/diff and apply the best-fit label on GitHub via `gh pr edit <number> --repo dknowles2/<repo> --add-label "<label>"` (use `bug` for fixes, `enhancement` for features, `documentation` for docs gaps, `chore` for maintenance).
